...
class TesseractNormalizer:
    def __init__(self):
        ...
        self.rules = [
            ...
            (re.compile(r"Г?[ГТ]?УБЕ\.?Р[НИ]А ?ТО ?Р"), "ГУБЕРНАТОР"),
            (re.compile(r"[НПп][РЕе]А[Вв]И[Тт]Е[ЛП]ЬСТВО"), "ПРАВИТЕЛЬСТВО"),
            (re.compile(r"ямало[ \-]?\n?[нп]ене(?:[цнп]к| |\n+)", re.I), "Ямало-Ненецк"),
            (re.compile(r"с?[ вё]?[вер][;ьръі][’›]?дл[оь]вской", re.I), "Свердловской"),
            (re.compile(r"[хж]а[нв]ты-?[ \n]?ма[ни]сийского", re.I), "Ханты-Мансийского"),
            (re.compile(r"обпаст", re.I), "област"),
            (re.compile(r"чепябинск", re.I), "Челябинск"),
            (re.compile(r"куз[ве]асс", re.I), "кузбасс"),
            (re.compile(r"кеме[юнл]?ов", re.I), "кемеров")
        ]
